...
- **Date:** 2026-09-19
- **Status:** Accepted
- **Decision:** The independent glycol loop's expansion vessel is a WFT 5 L expansion/header tank — Water Flow Tank brand (heatso.com, MPN 65288A; retrofit-compatible with Webasto/Eberspächer 29 2100 01 77 41), owner already has it on hand. **Not yet mounted.** The top of the tank must be the **highest circulating glycol point**. **Owner (2026-09-20):** WFT mounts in the **sofa-bed backrest (high)**; the Isotemp Slim Square 4.2 gal heat battery stays **under the bench/bed (low)** — same furniture stack, different heights (prefer driver side with the Isotemp). Fill/bleed through a hinged or removable backrest panel. Confirm height after framing; raise the backrest cradle or fall back to a higher wall mount only if the backrest loses the height check. Planned elevation order of magnitude ~20–30 in above the van floor is a starting guess, not an as-built fact.
...
## DEC-014 — SC1600B is a cabin dry-contact call; master remains the hard off {#dec-014}

- **Date:** 2026-09-19 (corrected 2026-09-20)
- **Status:** Accepted — **heater wake landing still OPEN (Q-022)**
- **Decision:** The SC1600B (Sure Marine 531-SO193701) is a space-heating control. It runs on AA batteries. Terminals R–W are a dry contact that closes below cabin setpoint. That contact will eventually (a) drive a **relay** so cabin fans only run when heat is called, and (b) wake the Hydronic — but **not** by landing on EasyStart Timer pins 9–10. Those pins are for Eberspächer’s optional temperature **sensor** (SRC-003), not a dry-contact thermostat. Approved wake landing stays under Q-022. Fan dial remains Low/Med/High speed only (DEC-015 / DEC-018). Master Off/On (DEC-012) still removes 12 V from EasyStart, the thermostat signal path, and the fans together.
- **Reason:** Owner wants cabin temperature to auto-call heat and fans, with one unambiguous hard off when parked. Sure Marine craft (SRC-019) treats these thermostats as low-current signals that need a relay for fans. EasyStart’s published Hydronic diagram does not show a dry-contact thermostat input on pins 9–10.
- **Alternatives considered:** Wiring SC1600 into EasyStart pins 9–10 as if it were the factory room sensor (rejected — wrong electrical type); claiming auto complete before Q-022 (rejected).
- **Supporting sources:** Owner 2026-09-19; SRC-003; SRC-019; SRC-033; Q-022
- **Affected documents:** `09-electrical-and-controls.md`, `book/01-what-this-system-does.md`, `book/03-living-with-it.md`, `open-questions.md`
...
- **Date:** 2026-09-20
- **Status:** Accepted
- **Decision:** Rebuild the sofa/bed as a **hybrid**: fixed wood side boxes (plywood + dimensional lumber), a short **aluminum** Isotemp cradle/restraint bolted to floor structure, and a **light plywood** sleep deck of panels that **lay flat by night**. By day (owner intent): **outboard panels become seat backs**, **inboard panels become seat butts**, with a walk-through aisle. Do **not** build the whole furniture in T-slot extrusion; do **not** rely on all-wood alone for Isotemp crash restraint. Exact panel count and hinge/slide hardware remain OPEN until measure.
- **Reason:** Weighted against easy-to-work (**35%**), strength (**25%**), light (**20%**), cost (**20%**) — hybrid scored **4.55 / 5** vs all-wood 4.15, all-extrusion 3.30, steel 2.75. Matches owner priority (access + construction ease). Owner revised day motion away from a carry-out full-length lift-out toward stay-attached panels that form seat backs and butts (2026-09-20). Field check: full aluminum bed frames save only ~7 lb over wood at ~3× frame cost; plywood deck dominates mass either way.
- **Alternatives considered:** All wood (rejected for tank restraint / revise); all 80/20 furniture (rejected — cost, rattle, slow first build); steel tube (rejected — weight / revise); Baltic birch everywhere (optional for visible faces only, not required as structure); one full-length lift-out + rear stash pocket (superseded by multi-panel seat-back / seat-butt geometry).
