...
Identify candidate hydronic architectures and which sources apply to each. Heater is Hydronic D5S diesel heater (DEC-000). Isotemp Slim Square 4.2 gal heat battery is the buffer (DEC-007). Duda B3-12DW-20 plate heat exchanger is the freshwater heat exchanger (DEC-003).
...
- **750 W element on the Isotemp Slim Square 4.2 gal heat battery:** separate Paneltronics AC load. It heats the static chamber only; it is not the DHW exchanger and does not replace the D5S circulating pump.
